...
module.exports = {
    ...
    execute: async function (interaction, client) {
        if (cooldowns.has(interaction.user.id)) {
            const remaining = Math.ceil((cooldowns.get(interaction.user.id) - Date.now()) / 1000);
            return interaction.reply({ content: `You can buy another scratch card in **${remaining}s**.`, flags: MessageFlags.Ephemeral });
        }
